...
WeightStabilizedSmallStrainMechanicsNOSPD::validParams()
{
  ...
  params.addParam<Real>(
      ...
      8.0,
      "The power of the weight function used to stabilized the correpondence model");
  ...
  for (unsigned int nd = 0; nd < _nnodes; ++nd)
  {
    ...
    for (unsigned int nb = 0; nb < neighbors.size(); ++nb)
                   ...
                   // considered in the local residual
      {
        ...
        Real len_ratio = std::abs(_origin_vec.norm() - origin_vec_nb.norm()) / _horizon_radius[nd];
  ...
  for (unsigned int nd = 0; nd < _nnodes; ++nd)
  {
    ...
    for (unsigned int nb1 = 0; nb1 < neighbors.size(); ++nb1)
      if (_bond_status_var->getElementalValue(_pdmesh.elemPtr(bonds[nb1])) > 0.5)
      {
        ...
        origin_vec_nb1 =
            _pdmesh.getNodeCoord(neighbors[nb1]) - _pdmesh.getNodeCoord(_current_elem->node_id(nd));
        ...
        Real len_ratio1 =
            std::abs(_origin_vec.norm() - origin_vec_nb1.norm()) / _horizon_radius[nd];
        Real cos_angle1 = inner_prod_nb1 / (_origin_vec.norm() * origin_vec_nb1.norm());
        if (cos_angle1 > 1.0)
        {
          cos_angle1 = 1.0;
        }
        else if (cos_angle1 < -1.0)
        {
          cos_angle1 = -1.0;
        }
        weight_nb1 = std::exp(-_pow * len_ratio1) * std::pow(0.5 + 0.5 * cos_angle1, _pow);
        ...
        for (unsigned int nb2 = 0; nb2 < neighbors.size(); ++nb2)
          if (_bond_status_var->getElementalValue(_pdmesh.elemPtr(bonds[nb2])) > 0.5)
          {
            ...
            origin_vec_nb2 = _pdmesh.getNodeCoord(neighbors[nb2]) -
            ...
            Real len_ratio2 =
                std::abs(_origin_vec.norm() - origin_vec_nb2.norm()) / _horizon_radius[nd];
            Real cos_angle2 = inner_prod_nb2 / (_origin_vec.norm() * origin_vec_nb2.norm());
            if (cos_angle2 > 1.0)
            {
              cos_angle2 = 1.0;
            }
            ...
            weight_nb2 = std::exp(-_pow * len_ratio2) * std::pow(0.5 + 0.5 * cos_angle2, _pow);
    ...
    for (unsigned int nd = 0; nd < _nnodes; ++nd)
    {
      ...
      for (unsigned int nb1 = 0; nb1 < neighbors.size(); ++nb1)
        if (_bond_status_var->getElementalValue(_pdmesh.elemPtr(bonds[nb1])) > 0.5)
        {
          ...
          origin_vec_nb1 = _pdmesh.getNodeCoord(neighbors[nb1]) -
                           _pdmesh.getNodeCoord(_current_elem->node_id(nd));
