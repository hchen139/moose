...
#include "OSPDKernel.h"

registerMooseObject("PeridynamicsApp", OSPDKernel);

template <>
InputParameters
validParams<OSPDKernel>()
{
...
OSPDKernel::OSPDKernel(const InputParameters & parameters)
...
void
OSPDKernel::computeLocalResidual()
{
  // H term
...
void
OSPDKernel::computeNonlocalResidual()
{
  // P and Q terms
  for (unsigned int cur_nd = 0; cur_nd < 2; ++cur_nd)
  {
    ...
    for (unsigned int k = 0; k < neighbors.size(); ++k)
    {
      ...
      // if (origin_ori_ijk.norm() > 1.0001 * _pdmesh.horizon(_nodes_ij[cur_nd]->id()))
      //   _local_re(cur_nd == 0 ? 0 : 1) = 0;
      // else if (origin_ori_ijk.norm() > 1.0001 * _pdmesh.horizon(node_k->id()))
      //   _local_re(cur_nd == 0 ? 1 : 0) = 0;
...
void
OSPDKernel::computeLocalJacobian()
{
  Real val =
...
void
OSPDKernel::computeLocalOffDiagJacobian(unsigned int coupled_component)
{
...
void
OSPDKernel::computeNonlocalJacobian()
{
...
void
OSPDKernel::computePDNonlocalOffDiagJacobian(unsigned int jvar_num, unsigned int coupled_component)
{
  ...
  for (unsigned int cur_nd = 0; cur_nd < _nnodes; ++cur_nd)
  {
    ...
    for (unsigned int k = 0; k < neighbors.size(); ++k)
    {
      ...
      _local_ke.zero();
      Real val = -_bond_force_i_j[cur_nd] * cur_ori_ijk(_component) *
                 cur_ori_ijk(coupled_component) / cur_ori_ijk.norm() / origin_ori_ijk.norm() *
                 vol_k;
      for (_i = 0; _i < _test.size(); ++_i)
        for (_j = 0; _j < _phi.size(); ++_j)
          _local_ke(_i, _j) += (_i == _j ? 1 : -1) * val * _bond_status_ij * bond_status_ijk;
