...
PDMechanicKernel::PDMechanicKernel(const InputParameters & parameters)
  : DerivativeMaterialInterface<PDKernelBase>(parameters),
...
{
  PDKernelBase::prepare();

  _ivardofs_ij.resize(2);
  _cur_ori_ij(_dim);
...
void
PDMechanicKernel::computeOffDiagJacobian(MooseVariableFEBase & jvar)
{
  prepare();
