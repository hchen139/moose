...
PDKernelBase::PDKernelBase(const InputParameters & parameters)
  : Kernel(parameters),
...
void
PDKernelBase::computeResidual()
{
  prepare();

  DenseVector<Number> & re = _assembly.residualBlock(_var.number());
  mooseAssert(re.size() == 2, "Edge2 element has only two nodes");
