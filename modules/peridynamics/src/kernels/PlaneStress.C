...
registerMooseObject("PeridynamicsApp", PlaneStress);

template <>
InputParameters
validParams<PlaneStress>()
{
...
PlaneStress::PlaneStress(const InputParameters & parameters) : SNOSPDKernelBase(parameters) {}

void
PlaneStress::computeLocalResidual()
{
  _local_re(0) =
...
void
PlaneStress::computeLocalJacobian()
{
...
void
PlaneStress::computeLocalOffDiagJacobian(unsigned int coupled_component)
{
  _local_ke.zero();
...
void
PlaneStress::computePDNonlocalOffDiagJacobian(unsigned int jvar_num, unsigned int coupled_component)
{
