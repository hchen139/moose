...
#include "GeneralizedPlaneStrainPDAction.h"
#include "NonlinearSystemBase.h"
#include "Factory.h"
#include "FEProblemBase.h"
#include "MooseObjectAction.h"

registerMooseAction("PeridynamicsApp", GeneralizedPlaneStrainPDAction, "add_kernel");
registerMooseAction("PeridynamicsApp", GeneralizedPlaneStrainPDAction, "add_user_object");
registerMooseAction("PeridynamicsApp", GeneralizedPlaneStrainPDAction, "add_scalar_kernel");

template <>
InputParameters
validParams<GeneralizedPlaneStrainPDAction>()
{
  InputParameters params = validParams<Action>();
  params.addClassDescription(
      "Class for setting up the peridynamic GeneralizedPlaneStrainPD action");
  ...
  params.addRequiredParam<AuxVariableName>("bond_status",
                                           "Auxiliary variable for the bond failure status");
  ...
  params.addParam<AuxVariableName>("out_of_plane_stress_variable",
                                   "Auxiliary variable name for out-of-plane stress");
  ...
  params.addParam<bool>(
      "full_jacobian",
      false,
      "Whether to use the nonlocal full jacobian formulation for the scalar components");
  params.addParam<bool>(
      "use_displaced_mesh", false, "Whether or not to use the displaced mesh for computation");
  params.addParam<std::vector<SubdomainName>>("block",
                                              "The list of ids of the blocks (subdomain) that the "
                                              "GeneralizedPlaneStrainPD kernels will be applied "
                                              "to");
...
GeneralizedPlaneStrainPDAction::GeneralizedPlaneStrainPDAction(const InputParameters & params)
  : Action(params),
...
void
GeneralizedPlaneStrainPDAction::act()
{
  if (_current_task == "add_kernel")
  {
    ...
    {
      case 0:
        k_type = "GeneralizedPlaneStrainOSPDOffDiag";
        break;
      case 1:
        k_type = "GeneralizedPlaneStrainSNOSPDOffDiag";
        break;
      default:
  ...
  else if (_current_task == "add_user_object")
  {
    ...
    {
      case 0:
        uo_type = "GeneralizedPlaneStrainOSPDUserObject";
        break;
      case 1:
        uo_type = "GeneralizedPlaneStrainSNOSPDUserObject";
        break;
      default:
        mooseError("Unsupported PD formulation. Choose from: OrdinaryState or NonOrdinaryState");
    }
  ...
  else if (_current_task == "add_scalar_kernel")
  {
    ...
    mooseError("Task error in GeneralizedPlaneStrainPDAction");
}
