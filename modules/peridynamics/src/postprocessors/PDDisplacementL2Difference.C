...
#include "PDDisplacementL2Difference.h"
#include "MooseVariableFEBase.h"
#include "Function.h"

registerMooseObject("PeridynamicsApp", PDDisplacementL2Difference);

template <>
InputParameters
validParams<PDDisplacementL2Difference>()
{
  InputParameters params = validParams<PDNodalIntegralPostprocessor>();
  params.addClassDescription("Class to compute the L2 norm of the difference between "
...
PDDisplacementL2Difference::PDDisplacementL2Difference(const InputParameters & parameters)
...
Real
PDDisplacementL2Difference::getValue()
{
  return std::sqrt(PDNodalIntegralPostprocessor::getValue());
}

Real
PDDisplacementL2Difference::computeNodalValue()
{
  Real diff = 0;
  if (_has_func_0)
  {
