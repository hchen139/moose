...
#include "NodalFunctionsL2Norm.h"
#include "Function.h"

registerMooseObject("PeridynamicsApp", NodalFunctionsL2Norm);

template <>
InputParameters
validParams<NodalFunctionsL2Norm>()
{
  InputParameters params = validParams<PDNodalIntegralPostprocessor>();
  params.addClassDescription("Class to compute the L2 norm of function(s)");
...
NodalFunctionsL2Norm::NodalFunctionsL2Norm(const InputParameters & parameters)
...
Real
NodalFunctionsL2Norm::getValue()
{
  return std::sqrt(PDNodalIntegralPostprocessor::getValue());
}

Real
NodalFunctionsL2Norm::computeNodalValue()
{
  Real func_val = 0;
