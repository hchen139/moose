...
#include "NodalStressStrainAux.h"
#include "PeridynamicMesh.h"
#include "RankTwoScalarTools.h"

registerMooseObject("PeridynamicsApp", NodalStressStrainAux);

template <>
InputParameters
validParams<NodalStressStrainAux>()
{
  InputParameters params = validParams<AuxKernel>();
  params.addClassDescription("Class for computing and outputing components and scalar quantities "
                             "of nodal rank two strain/stress tensors for bond-based and ordinary "
  ...
  params.addParam<std::string>("rank_two_tensor",
                               "Which rank two tensor: total_strain, elastic_strain or stress");
...
NodalStressStrainAux::NodalStressStrainAux(const InputParameters & parameters)
...
Real
NodalStressStrainAux::computeValue()
{
  Real val = 0.0;
  ...
  if (_rank_two_tensor == "total_strain")
    ...
    mooseError("NodalStressStrainAux Error: Pass valid rank_two_tensor - total_strain, "
  ...
  switch (_quantity_type)
  {
    ...
    default:
      mooseError(
          "NodalStressStrainAux Error: Pass valid quantity type - Component or VonMisesStress");
  }
...
RankTwoTensor
NodalStressStrainAux::computeNodalTotalStrain()
{
  ...
  // TODO: need to check whether the inverse of shape_tensor is exist
  dgrad *= shape.inverse();
...
RankTwoTensor
NodalStressStrainAux::computeNodalElasticStrain()
{
...
RankTwoTensor
NodalStressStrainAux::computeNodalStress()
{
