...
template <>
InputParameters
validParams<PeridynamicMesh>()
{
  InputParameters params = validParams<MooseMesh>();
  params.addClassDescription("Base class for generating mesh for peridynamic simulation");

  params.addParam<Real>("horizon_radius", "Horizon size in terms of radius");
  params.addParam<Real>("horizon_number",
                        "The ratio number of horizon radius to the effective mesh spacing");
...
unsigned int
PeridynamicMesh::dimension() const
{
  return _dim;
}

dof_id_type
PeridynamicMesh::nNodes() const
{
  return _total_nodes;
}

dof_id_type
PeridynamicMesh::nElem() const
{
  return _total_bonds;
}

Real
PeridynamicMesh::computeHorizon(Real spacing)
{
...
PeridynamicMesh::findNodeNeighbor()
{
  for (unsigned int i = 0; i < _total_nodes; ++i)
  {
    ...
    for (unsigned int j = 0; j < _total_nodes; ++j)
    {
      ...
      // if (_pdnode[i].blockID == _pdnode[j].blockID && dis <= 1.0001 * _pdnode[i].horizon &&
      //     j != i && dis <= 1.0001 * _pdnode[j].horizon)
      if (_pdnode[i].blockID == _pdnode[j].blockID && dis <= 1.0001 * _pdnode[i].horizon && j != i)
      {
        ...
        if (!intersect)
        {
              ...
              _node_neighbors[j].end())
          {
            ...
            // if (dis <= 1.0001 * _pdnode[j].horizon)
      ...
      // if (_pdnode[i].blockID == _pdnode[j].blockID && dis <= 5.01 * _pdnode[i].mesh_spacing &&
      //     j != i)
      //   _node_n_nearest_neighbors[i].push_back(j);
    }
  }
...
void
PeridynamicMesh::setupDGNodeInfo()
{
  for (unsigned int i = 0; i < _total_nodes; ++i)
  {
    ...
    for (unsigned int j = 0; j < neighbors.size(); ++j)
    {
      ...
      // Point vec_ij = _pdnode[i].coord - _pdnode[neighbors[j]].coord;
      std::vector<dof_id_type> j_n_nearest_neighbors = _node_n_nearest_neighbors[neighbors[j]];
      // std::vector<dof_id_type> j_n_nearest_neighbors = _node_neighbors[neighbors[j]];
      for (unsigned int k = 0; k < j_n_nearest_neighbors.size(); ++k)
      {
        // Point vec_ik = _pdnode[i].coord - _pdnode[j_n_nearest_neighbors[k]].coord;
        // Point cross = vec_ij.cross(vec_ik);
        // Real dot = vec_ij * vec_ik / (vec_ij.norm() * vec_ik.norm());
        auto it = std::find(neighbors.begin(), neighbors.end(), j_n_nearest_neighbors[k]);
        // if (it != neighbors.end() && i != j_n_nearest_neighbors[k] &&
        //     !MooseUtils::absoluteFuzzyEqual(cross.norm(), 0.0))
        if (it != neighbors.end() && i != j_n_nearest_neighbors[k])
        {
...
unsigned int
PeridynamicMesh::neighborID(dof_id_type node_i, dof_id_type node_j)
{
...
PeridynamicMesh::dgNodeInfo(dof_id_type node_id, unsigned int neighbor_id)
{
...
Real
PeridynamicMesh::dgBondVolumeSum(dof_id_type node_id, unsigned int neighbor_id)
{
  return _dg_bond_volumesum[node_id][neighbor_id];
}

Real
PeridynamicMesh::dgNodeVolumeSum(dof_id_type node_id)
{
  return _dg_node_volumesum[node_id];
}

unsigned int
PeridynamicMesh::nneighbors(dof_id_type node_id)
{
...
bool
PeridynamicMesh::checkCrackIntersection(Point A, Point B, Point C, Point D, Real width)
{
...
bool
PeridynamicMesh::checkSegmentIntersection(Point A, Point B, Point C, Point D)
{
