...
FileMeshPD::FileMeshPD(const InputParameters & parameters) : PeridynamicMesh(parameters) {}
...
FileMeshPD::buildMesh()
{
  ...
  for (unsigned int i = 0; i < _total_nodes; ++i)
  {
    ...
    // if (std::abs(Y) < 0.001 && X < 0.0)
    if (std::abs(Y) < 0.0006)
      pd_boundary_info.add_node(pd_mesh.node_ptr(i), 102);
    if (std::abs(Z) < 0.001)
      pd_boundary_info.add_node(pd_mesh.node_ptr(i), 103);
    if (std::abs(X + 0.025) < 0.0006)
      pd_boundary_info.add_node(pd_mesh.node_ptr(i), 104);
    if (std::abs(X - 0.025) < 0.0006)
      pd_boundary_info.add_node(pd_mesh.node_ptr(i), 105);
    if (Y > 0.049 && std::abs(X) < 0.001)
      pd_boundary_info.add_node(pd_mesh.node_ptr(i), 106);
    if (Y < -0.049 && std::abs(X) < 0.001)
      pd_boundary_info.add_node(pd_mesh.node_ptr(i), 107);
  ...
  pd_boundary_info.nodeset_name(102) = "CenterLine";
  pd_boundary_info.nodeset_name(103) = "CenterPlane";
  pd_boundary_info.nodeset_name(104) = "LeftLine";
  pd_boundary_info.nodeset_name(105) = "RightLine";
  pd_boundary_info.nodeset_name(106) = "TopCenter";
  pd_boundary_info.nodeset_name(107) = "BottomCenter";
  pd_boundary_info.nodeset_name(999) = "All";

  _console << "Peridynamic Mesh Information:" << '\n';
