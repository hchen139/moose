...
ComputeForceStabilizedSmallStrainNOSPD::computeQpDeformationGradient()
{
  ...
  for (unsigned int j = 0; j < neighbors.size(); ++j)
    if (_bond_status_var.getElementalValue(_pdmesh.elemPtr(bonds[j])) > 0.5)
    {
      ...
      for (unsigned int k = 0; k < _dim; ++k)
      {
        ...
        {
          _shape_tensor[_qp](k, l) += vol_j * _horiz_size[_qp] / ori_len * ori_vec(k) * ori_vec(l);
          _deformation_gradient[_qp](k, l) +=
              vol_j * _horiz_size[_qp] / ori_len * cur_vec(k) * ori_vec(l);
        }
        // calculate derivatives of deformation_gradient w.r.t displacements of node i
        _ddgraddu[_qp](0, k) += -vol_j * _horiz_size[_qp] / ori_len * ori_vec(k);
        _ddgraddv[_qp](1, k) += -vol_j * _horiz_size[_qp] / ori_len * ori_vec(k);
        if (_dim == 3)
          _ddgraddw[_qp](2, k) += -vol_j * _horiz_size[_qp] / ori_len * ori_vec(k);
      }
    }
  ...
  _sf_coeff[_qp] = youngs_modulus * _pdmesh.getNodeAvgSpacing(_current_elem->node_id(_qp)) *
                   _horiz_size[_qp] / _origin_length;
  _multi[_qp] = _horiz_size[_qp] / _origin_length * _node_vol[0] * _node_vol[1];
}
