...
#include "CorrespondenceFNOSPD.h"

registerMooseObject("PeridynamicsApp", CorrespondenceFNOSPD);

template <>
InputParameters
validParams<CorrespondenceFNOSPD>()
{
  InputParameters params = validParams<CorrespondenceSmallStrain>();
  params.addClassDescription(
      "Class for computing bond interaction for peridynamic correspondence model");

  return params;
}

CorrespondenceFNOSPD::CorrespondenceFNOSPD(const InputParameters & parameters)
  : CorrespondenceSmallStrain(parameters),
...
void
CorrespondenceFNOSPD::computeQpDeformationGradient()
{
    ...
    for (unsigned int k = 0; k < _dim; ++k)
    {
      for (unsigned int l = 0; l < _dim; ++l)
      {
        _shape_tensor[_qp](k, l) +=
            vol_j * _horizon[_qp] / origin_length * origin_vec(k) * origin_vec(l) * bond_status_ij;
        _deformation_gradient[_qp](k, l) +=
            vol_j * _horizon[_qp] / origin_length * current_vec(k) * origin_vec(l) * bond_status_ij;
      }
      // calculate derivatives of deformation_gradient w.r.t displacements of node i
      _ddgraddu[_qp](0, k) +=
          -vol_j * _horizon[_qp] / origin_length * origin_vec(k) * bond_status_ij;
      _ddgraddv[_qp](1, k) +=
          -vol_j * _horizon[_qp] / origin_length * origin_vec(k) * bond_status_ij;
      if (_dim == 3)
        _ddgraddw[_qp](2, k) +=
  ...
  if (_deformation_gradient[_qp].det() == 0)
