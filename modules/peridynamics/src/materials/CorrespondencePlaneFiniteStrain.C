...
#include "CorrespondencePlaneFiniteStrain.h"
#include "RankTwoTensor.h"

registerMooseObject("PeridynamicsApp", CorrespondencePlaneFiniteStrain);

template <>
InputParameters
validParams<CorrespondencePlaneFiniteStrain>()
{
  InputParameters params = validParams<CorrespondenceFiniteStrain>();
...
CorrespondencePlaneFiniteStrain::CorrespondencePlaneFiniteStrain(const InputParameters & parameters)
  : CorrespondenceFiniteStrain(parameters),
...
void
CorrespondencePlaneFiniteStrain::computeQpFhat()
{
...
Real
CorrespondencePlaneFiniteStrain::computeQpOutOfPlaneDeformationGradient()
{
  /**
...
Real
CorrespondencePlaneFiniteStrain::computeQpOutOfPlaneDeformationGradientOld()
{
  if (_scalar_out_of_plane_strain_coupled)
