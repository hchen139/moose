...
template <>
InputParameters
validParams<BPDThermalMaterial>()
{
  ...
  params.addRequiredParam<NonlinearVariableName>("temperature",
                                                 "Variable refering to the temperature");
...
BPDThermalMaterial::BPDThermalMaterial(const InputParameters & parameters)
    ...
    _bond_dQdT(declareProperty<Real>("bond_dQdT"))
{
  if (_thermal_conductivity_function)
  {
    ...
    mooseError("Neither thermal_conductivity nor thermal_conductivity_function is provided for "
               "peridynamic thermal models: BPDThermalMaterial");
}

void
BPDThermalMaterial::computeProperties()
{
...
void
BPDThermalMaterial::computeBondHeatFlow()
{
  // residual term
...
void
BPDThermalMaterial::computeNodalTemperature()
{
