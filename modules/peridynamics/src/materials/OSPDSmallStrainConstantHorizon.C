...
#include "OSPDSmallStrainConstantHorizon.h"

registerMooseObject("PeridynamicsApp", OSPDSmallStrainConstantHorizon);

template <>
InputParameters
validParams<OSPDSmallStrainConstantHorizon>()
{
  InputParameters params = validParams<OSPDSmallStrainMaterial>();
  params.addClassDescription(
      "Class to compute peridynamic micro elastic moduli for ordinary state-based model "
      "using regular uniform mesh");

  return params;
}

OSPDSmallStrainConstantHorizon::OSPDSmallStrainConstantHorizon(const InputParameters & parameters)
  : OSPDSmallStrainMaterial(parameters)
{
}

void
OSPDSmallStrainConstantHorizon::computePDMicroModuli()
{
